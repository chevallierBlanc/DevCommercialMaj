...
Namespace DevCommerc8ak
    Public Class StockSortieNonSynchroniseRepository
        ...
        Public Function Ajouter(jsonData As String, Optional statutSync As String = "EN_ATTENTE", Optional messageErreur As String = Nothing) As Integer
            Dim sql As String = "INSERT INTO StockSortieNonSynchronise (JsonData, StatutSync, MessageErreur) VALUES (@JsonData, @StatutSync, @MessageErreur); SELECT CAST(SCOPE_IDENTITY() AS INT);"
        ...
        Public Function ListerEnAttente() As DataTable
            Dim sql As String = "SELECT Id, JsonData, DateCreation, NombreTentatives, DerniereTentative, StatutSync, MessageErreur FROM StockSortieNonSynchronise WHERE StatutSync <> 'SYNC_OK' ORDER BY DateCreation ASC"
        ...
        Public Sub MarquerResultat(id As Integer, statutSync As String, messageErreur As String, nombreTentatives As Integer)
            Dim sql As String = "UPDATE StockSortieNonSynchronise SET StatutSync=@StatutSync, MessageErreur=@MessageErreur, NombreTentatives=@Nb, DerniereTentative=SYSUTCDATETIME() WHERE Id=@Id"
            Dim p As New List(Of SqlParameter) From {
                New SqlParameter("@StatutSync", statutSync),
                New SqlParameter("@MessageErreur", If(String.IsNullOrWhiteSpace(messageErreur), CType(DBNull.Value, Object), messageErreur)),
