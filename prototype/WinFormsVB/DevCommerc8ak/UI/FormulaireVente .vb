    ...
    Public Class FormulaireVente
        ...
        Private Sub ExporterPdfVentes(sender As Object, e As EventArgs)
            ...
            Using sfd As New SaveFileDialog()
                ...
                pdocVentes.DefaultPageSettings.Margins = New Margins(30, 30, 30, 30)
                Try
        ...
        Private Sub ExporterPdfStock(sender As Object, e As EventArgs)
            ...
            Using sfd As New SaveFileDialog()
                ...
                pdocStock.DefaultPageSettings.Margins = New Margins(30, 30, 30, 30)
                Try
        ...
        Private Sub JournaliserErreurExportPdf(action As String, ex As Exception)
            Try
                ...
                log.Error("FormulaireVente", action, "Erreur lors de l'export PDF.", ex)
            Catch
            End Try
        End Sub
        ...
        Private Function ObtenirZoneRapport(e As PrintPageEventArgs, doc As PrintDocument, ratioLargeur As Single) As RectangleF
            Dim zoneImprimable As RectangleF = ObtenirZoneImprimable(e, doc)
            Dim ratio As Single = Math.Max(0.9F, Math.Min(0.96F, ratioLargeur))
            Dim largeurRapport As Single = CSng(Math.Floor(zoneImprimable.Width * ratio))
        ...
        Private Sub PdocVentes_PrintPage(sender As Object, e As PrintPageEventArgs)
            ...
            Dim zoneRapport As RectangleF = ObtenirZoneRapport(e, TryCast(sender, PrintDocument), 0.94F)
            Dim left As Integer = CInt(Math.Round(zoneRapport.Left))
            Dim top As Integer = CInt(Math.Round(zoneRapport.Top))
            Dim largeur As Integer = CInt(Math.Round(zoneRapport.Width))
            ...
            Using titreFont As New Font("Segoe UI", 16.0F, FontStyle.Bold),
                    ...
                    For i As Integer = 0 To colonnes.Length - 1
                        ...
                        If Not row.IsNull(colonnes(i)) Then
                            Select Case colonnes(i)
                                Case "DateVente"
                                    valeurs(i) = Convert.ToDateTime(row(colonnes(i))).ToString("dd/MM/yyyy HH:mm")
                                Case "Produit"
                                    valeurs(i) = Convert.ToString(row(colonnes(i)))
                                Case "QuantiteVenduePieces"
                                    valeurs(i) = FormaterQuantiteBase(row, colonnes(i), False)
                                Case Else
                                    valeurs(i) = Convert.ToDecimal(row(colonnes(i))).ToString("N0")
                            End Select
                        End If
                    Next
                ...
                While _ventePrintRowIndex < data.Rows.Count
                    ...
                    If y + hauteurLigne > limiteBasTableau AndAlso lignesImprimees > 0 Then
                        e.Graphics.DrawString("Page " & _ventePrintPageIndex.ToString(), sousTitreFont, pinceauGris, left + largeur - 80, footerY)
                        e.HasMorePages = True
                        _ventePrintPageIndex += 1
                        Return
                    End If
                    ...
                    x = left
                    For i As Integer = 0 To valeurs.Length - 1
                        DessinerCelluleTableau(e.Graphics, valeurs(i), ligneFont, Brushes.Black, bordure, x, y, largeurs(i), hauteurLigne, If(i >= 2, sfRight, sfLeft))
                        x += largeurs(i)
                    Next
        ...
        Private Sub PdocStock_PrintPage(sender As Object, e As PrintPageEventArgs)
            ...
            Using titreFont As New Font("Segoe UI", 16.0F, FontStyle.Bold),
                    ...
                    For i As Integer = 0 To colonnes.Length - 1
                        ...
                        If Not row.IsNull(colonnes(i)) Then
                            ...
                            ElseIf String.Equals(colonnes(i), "StockActuelPieces", StringComparison.OrdinalIgnoreCase) OrElse
                                ...
                                valeurs(i) = FormaterQuantiteBase(row, colonnes(i), True)
                            ElseIf String.Equals(colonnes(i), "StockActuelCartons", StringComparison.OrdinalIgnoreCase) Then
                                valeurs(i) = FormaterPresentationPrincipale(row, colonnes(i))
                ...
                While _stockPrintRowIndex < data.Rows.Count
                    ...
                    Dim hauteurLigne As Integer = MesurerHauteurLigneTableau(e.Graphics, ligneFont, valeurs, largeurs, hauteurLigneMin)
                    If y + hauteurLigne > limiteBasTableau AndAlso lignesImprimees > 0 Then
                        e.Graphics.DrawString("Page " & _stockPrintPageIndex.ToString(), sousTitreFont, pinceauGris, left + largeur - 80, footerY)
                        e.HasMorePages = True
                        _stockPrintPageIndex += 1
                        Return
                    End If
                    ...
                    x = left
                    For i As Integer = 0 To valeurs.Length - 1
                        DessinerCelluleTableau(e.Graphics, valeurs(i), ligneFont, Brushes.Black, bordure, x, y, largeurs(i), hauteurLigne, If(i = 0, sfLeft, sfRight))
                        x += largeurs(i)
                    Next
        ...
        Private Sub ExporterPdfDepenses(sender As Object, e As EventArgs)
            ...
            Using sfd As New SaveFileDialog()
                ...
                pdocDepenses.DefaultPageSettings.Margins = New Margins(30, 30, 30, 30)
                Try
        ...
        Private Sub PdocDepenses_PrintPage(sender As Object, e As PrintPageEventArgs)
            ...
            Dim zoneImprimable As RectangleF = ObtenirZoneImprimable(e, TryCast(sender, PrintDocument))
            Dim zoneRapport As RectangleF = ObtenirZoneRapport(e, TryCast(sender, PrintDocument), 0.92F)
            Dim left As Integer = CInt(Math.Round(zoneRapport.Left))
            Dim pageWidth As Integer = CInt(Math.Round(zoneRapport.Width))
            ...
            Using titreFont As New Font("Segoe UI", 16.0F, FontStyle.Bold),
                    ...
                    For i As Integer = 0 To colonnes.Length - 1
                        ...
                        If Not row.IsNull(colonnes(i)) Then
                            If String.Equals(colonnes(i), "MontantTotal", StringComparison.OrdinalIgnoreCase) Then
                                valeurs(i) = FormatageGlobal.FormatMontant(Convert.ToDecimal(row(colonnes(i))))
                            ...
                            ElseIf String.Equals(colonnes(i), "PremiereDate", StringComparison.OrdinalIgnoreCase) OrElse String.Equals(colonnes(i), "DerniereDate", StringComparison.OrdinalIgnoreCase) Then
                                valeurs(i) = Convert.ToDateTime(row(colonnes(i))).ToString("dd/MM/yyyy")
                            Else
                                valeurs(i) = Convert.ToString(row(colonnes(i)))
                            End If
                        End If
                ...
                While _depensePrintRowIndex < data.Rows.Count
                    ...
                    Dim hauteurLigne As Integer = MesurerHauteurLigneTableau(e.Graphics, ligneFont, valeurs, largeurs, hauteurLigneMin)
                    If y + hauteurLigne > limiteBasTableau AndAlso lignesImprimees > 0 Then
                        e.Graphics.DrawString("Page " & _depensePrintPageIndex.ToString(), sousTitreFont, pinceauGris, left + pageWidth - 80, footerY)
                        e.HasMorePages = True
                        _depensePrintPageIndex += 1
                        Return
                    End If
